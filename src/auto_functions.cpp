...
void auto_drive(int direction, int &step, int target, int start_power, double precision, const double accel_vars[], const double deccel_vars[], int max_power, int time_limit) {
  ...
  if (step == 0) {
    ...
    if ((getTime(AUTO_DRIVE_TIMEOUT) % 10) == 0) {
...
void auto_turn(int direction, int &step, int raw_target, int start_power, const double precision, const double accel_vars[], double deccel_vars[], int max_power, int time_limit) {
  ...
  if (step < AUTO_TURN_MAX_STEP && getTime(AUTO_TURN_TIMEOUT) <= time_limit) {
    switch (step) {
      case 0 :
      // setup
      Auto_Turn_PID.set_PID_variables(half_target, abs(max_power), -abs(max_power), accel_vars[AUTO_TURN_I_LIM]);
      ...
      */
      ...
      // calculate power based on output
      power_PID = Auto_Drive_PID.output(gyro.get_value());
      power_diff = (max_power * direction) - power_PID;
      power_drive = (power_diff < start_power) ? start_power : power_diff;
      ...
      // move to next step
      // if the value of the drive is more than (2 * precision)% to the halfway point or half of the time limit have passed then move to the next step
      if ((fabs(gyro.get_value()) > (abs(half_target) - (abs(half_target) * (precision * 2)))) || (getTime(AUTO_TURN_STEP_EXIT) >= (time_limit / 2))) {
        step++;
        Auto_Drive_PID.set_PID_variables((abs(target) * direction), (abs(max_power)), (-(abs(max_power))), deccel_vars[AUTO_TURN_I_LIM]);
        Auto_Drive_PID.set_PID_constants(deccel_vars[AUTO_TURN_KP], deccel_vars[AUTO_TURN_KI], deccel_vars[AUTO_TURN_KD]);
        startTimer(AUTO_TURN_STEP_EXIT);
      }
      break;
      case 2 :
      ...
      // move to next step
      // if the value of the drive is more than (precision)% to the halfway point or half of the time limit have passed then move to the next step
      if ((fabs(gyro.get_value()) > (abs(target) - (abs(target) * precision))) || (getTime(AUTO_TURN_STEP_EXIT) >= (time_limit / 2))) {
        step++;
      }
      break;
    }
